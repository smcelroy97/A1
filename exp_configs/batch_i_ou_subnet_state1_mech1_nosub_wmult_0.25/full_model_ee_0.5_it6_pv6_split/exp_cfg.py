...
from batch_params import CONN_FRAC_ACTIVE_VALS
...
def apply_exp_cfg(cfg, par=None):
    ...
    conns_split = {f'{c[0]}, {c[1]}': 0.5 for c in conns_ee}
    conns_split['IT6, PV6'] = None   # will be set in batch_params.py
    ...
    cfg.conn_frac_active = None   # batch parameter
...
def post_run(sim):
    ...
    # Experiment sub-name
    kmin, kmax = CONN_FRAC_ACTIVE_VALS[0], CONN_FRAC_ACTIVE_VALS[-1]
    exp_name_sub = (f'exp_t_{t_limits[0]}_{t_limits[1]}_'
                    f'wfrac_{kmin}_{kmax}')
    if cfg.add_pulses:
    ...
    postfix = (f'{exp_id}_wfrac_{cfg.conn_frac_active}')
