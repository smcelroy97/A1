...
def apply_exp_cfg(cfg):
    ...
    # Populations to use
    pops_active = ['VIP2', 'VIP3', 'VIP4', 'VIP5A', 'VIP5B', 'VIP6']
    ...
    if ncells_plot:
        cfg.analysis['plotTraces'] = {
            ...
            'timeRange': [cfg.duration - 3000, cfg.duration],
...
def post_run(sim):
    ...
    fpath_old_lst = dirpath_res.glob(f'{exp_name}*traces*.png')
    for n, fpath_old in enumerate(fpath_old_lst):
        #fpath_old = dirpath_res / f'{exp_name}_traces.png'
        fpath_new = dirpath_res_sub / 'traces' / f'traces_{postfix}_{n}.png'
        if fpath_old.exists():
            fpath_old.rename(fpath_new)
