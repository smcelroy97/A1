...
from batch_params import BATCH_PARAMS, gen_exp_name_sub, _select_ou_from_range
...
def post_batch(exp_name):
    ...
    for n, ou_mean_pos in enumerate(job_idx_xr.ou_mean_pos.values):
        for m, ou_std_pos in enumerate(job_idx_xr.ou_std_pos.values):
            ...
            ou_pop_inputs = _select_ou_from_range(ou_mean_pos, ou_std_pos)
    ...
    for n in range(4):
        ...
        plt.colorbar()
    # Save the figure
    plt.savefig(dirpath_comb / 'result.png', dpi=300)


if __name__ == '__main__':
    exp_name = 'batch_i_ougrid_unconn_state1_nosub_wmult_0.1_1pop/pv2'
    post_batch(exp_name)
