...
def post_run(sim):
    ...
    for t_limits in time_ranges:
        V_, t_ = parse_utils.get_voltages(
            sim_result, np.array(t_limits) * 1000
        )
        V.append(V_[pop])
        t.append(t_)
    #cell_idx = [v[1] for v in sim.cfg.recordCells if v[0] == pop][0]
    ...
    #fname_out = f'v_{postfix}.nc'
    #V_data.to_netcdf(dirpath_res / 'voltages' / fname_out)
