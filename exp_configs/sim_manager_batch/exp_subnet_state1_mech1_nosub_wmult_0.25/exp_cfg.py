...
def apply_exp_cfg(cfg):
    ...
    # Duration
    cfg.duration = 10 * 1e3   # can be changed in a SimManager request

    # Subnet parameters
    cfg.subnet_build_flag = 1
    cfg.subnet_params = {
        'pops_active': [],   # will be taken from a request generated by SimManager
        'conns_frozen': 'all',   # all inputs are surrogate, no recurrent connections,
                                 # can be changed in a SimManager request
