...
def apply_exp_cfg(cfg, par=None):
    ...
    if OU_CTRL:
        cfg.ou_ctrl_params = {
            'mu0': {pop: inp['ou_mean'] for pop, inp in cfg.ou_pop_inputs.items()},
            'sigma0': {pop: inp['ou_std'] for pop, inp in cfg.ou_pop_inputs.items()},
            'mu_gain': 0,
            'sigma_gain': 0,
            'tau_ctrl': 100,
...
def post_run(sim):
    ...
    res_names = ['raster.png', 'cfg.json', 'netParams.json',
