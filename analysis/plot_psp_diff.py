...
for syn_type in ['E', 'I']:
    ...
    plt.title(f'Amplitude Difference Heatmap with gain - without gain (syn_type={syn_type})')
    plt.xlabel('prePop')
    plt.ylabel('pop')
    plt.show()
...
for syn_type in ['E', 'I']:
    ...
    plt.figure(figsize=(10, 8))
    sns.heatmap(heatmap_data, annot=False, cmap='viridis', center=0)
    plt.title(f'Amplitude Difference Heatmap wmat - no gain weights (syn_type={syn_type})')
    plt.xlabel('prePop')
    plt.ylabel('pop')
    plt.show()