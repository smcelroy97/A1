...
if sim.rank == 0:
    ...
    spikes = [(gid, t) for gid, t in zip(spike_gids, spike_times)]

    results = {}
    for pop in Ipops + Epops:
        pop_gids = [gid for gid, tags in cell_tags.items() if tags['pop'] == pop]
        pop_spikes = [gid for gid, t in spikes if gid in pop_gids]
        rate = len(pop_spikes) / (len(pop_gids) * cfg.duration) if len(pop_gids) > 0 else 0
        results[pop] = rate
    ...
    def fitnessFunc(rates, **kwargs):
        pops = kwargs['pops']
        maxFitness = kwargs['maxFitness']
        popFitness = [min(np.exp(abs(v['target'] - rates[k]) / v['width']), maxFitness)
                      if rates[k] > v['min'] else maxFitness for k, v in pops.items()]
        fitness = np.mean(popFitness)

        popInfo = '; '.join(
            ['%s rate=%.1f fit=%1.f' % (p, rates[p], popFitness[i]) for i, p in enumerate(pops)])
        print('  ' + popInfo)
        return fitness

    results['loss'] = fitnessFunc(results, **fitnessFuncArgs)
